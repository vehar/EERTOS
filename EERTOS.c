...
//#undef DEBUG
/*
Если таймер установить на 50-100мкс при 16МГц?

Плохо будет работать. Многое можно оптимизировать,
среднее выполнение блока таймерной службы сейчас около 600 тактов.
Столько же в среднем занимает перебор очереди,
...
Реально тайминг снизить до 500мкс ,но делать очень короткие задачи, иначе таймер сбоить будет.
*/
...
//+++++++++++++PRIVATE RTOS SERVICES++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  void TaskSetToNULL(uint16_t index); //зануление атрибутов задачи по индексу
...
//===============================================================================================
 ...
 void Shedull (uint8_t param)
 {
	 while(1)
	 {
	 HandlerProfiling_Start(); //TODO: выяснить странность статистики
		  TaskManager();
	...
	if((v_u32_SYS_TICK % QUEUE_SORTING_PERIOD) == 0)//За 1 тик можно попасть сюда много раз!
        {
          ...
          Abs_in_Idle_time = (avg_idle_time * Idle_statts_cnt_global)/QUEUE_SORTING_PERIOD/72;//72 - sys clk MHz
...
//===============================================================================================
void TaskManager(void) //в диспетчере задач выполняются задачи из очереди TTask, которые уже "выщелкали"
{
U_ALU_INT	index = 0;
T_PTR 		CurrentTask;
...
U_ALU_INT	result = QUEUE_FULL;
 ...
 {
  FOREACH_TASK	// Поиск следующей доступной позиции в массиве задач
  {
   ...
   //+++++++++++++  //Шейкерная сортировка
           ...
           while(l <= r)
           {
              for(index = l; index <= r; index++)
              {
                 ...
                 {
                 tmp = TTask[index];   TTask[index] = TTask[index+1];
...
{
#warning Strongly not recomended to use!!! -возможность пробить стек при большом кол-ве вызовов!
...
//TODO: можно добавить проверку на удаление задачи или изменение статуса извне, тогда ошибка
 ...
 {
 	while (v_u32_SYS_TICK < tick_cnt) //пока не выщелкали всю задержку
	{ //похоже в этом месте sys-tick останавливается!
#ifdef USE_UINT16_T_TASK_VS_T_ARG_PARAMETER	 
		Idle(0); //..тупим в Idle
#endif		
#ifdef USE_VOID_TASK_VS_VOID_PARAMETER	 
...
#warning wtf with inline шт KEIL_5???
/*inline*/ void HandlerProfiling_End(void)
{
uint32_t index = 0;
...
//Дебажные выводы
 //===============================================================================================
#ifdef DEBUG
//TODO: добавить привязку имени задачи к её адресу и нормальный вывод в терминал
...
/*
T_PTR GoToTask; 				// Указатель перехода
uint16_t TaskDelay;				// Выдержка в мс перед стартом задачи
